...
const isUserMemberOfWorkspace = async (workspace, userId) => {
    return await workspace.members.find(member => member.memberId.toString() === userId);
}
...
export const deleteWorkspaceService = async(workspaceId, userId) => {
    try {
        ...
        if(isAllowed){
            const channelDeleteResponse = await workspaceRepository.deleteMany(workspace.channels);
            const response = await workspaceRepository.delete(workspaceId);
            return response;
        }
        throw new ClientError({
            explanation: 'User is either not a member of the workspace or not an admin',
...
export const addChannelToWorkspaceService = async (workspaceId, channelName, userId) => {
    try {
        ...
        const isChannelPartOfWorkspace = await isChannelAlreadyPartOfWorkspace(workspace, channelName);
